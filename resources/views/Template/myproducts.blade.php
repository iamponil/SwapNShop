...
<body class="animsition">
		...
		<div class="container">
			<div class="flex-w flex-sb-m p-b-52">
				<div class="flex-w flex-l-m filter-tope-group m-tb-10">
					...
					<a class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".shoes">
						Shoes
					</a>

					<a class="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5" data-filter=".watches">
						Watches
					</a>
				</div>
    ...
    @endif
			...
			<div class="flex-c-m flex-w w-full p-t-45">
				<a href="{{ route('create') }}" class="flex-c-m stext-101 cl5 size-103 bg2 bor1 hov-btn1 p-lr-15 trans-04">
					Add More Product
				</a>
			</div>
		</div>
...
<script>
    ...
    $('.show-product').click(function () {
		...
		$.ajax({
                type: 'GET',
                url: '/getProductOffers/' + productId,
                success: function (data) {
                    if ('offers' in data) {
                        var offers = data.offers;
                        var tableBody = $('#productOffers');
                        tableBody.empty();
						
						var exchangeMessage = $('<p>')
            .text('The exchange is made, this product is in the shipping phase being exchanged for a ')
            .append($('<span>').attr('id', 'productNameToExchange'))
            .append(' owned by ')
            .append($('<span>').attr('id', 'userNameOffering'))
            .hide();

        $('#exchangeMessage').replaceWith(exchangeMessage);
                        offers.forEach(function (offer) {
                            var username = offer.user.name;
                            var productName = offer.produit_a_echanger.product_name;

                            var row = $('<tr>');
                            var userCell = $('<td>').text(username);
                    ...
                    $('#userNameOffering').text(offer.user.name);			
                     }
                           actionCell.append(confirmButton, messageButton);
                            row.append(userCell, productCell, actionCell);
                            tableBody.append(row);
                        });
                    } else {
                        console.error('La structure JSON de la réponse est incorrecte.');
                    }
					if (hasConfirmedOffer) {
						$('.confirmer-button').remove();
        ...
        // Écoutez les clics sur les boutons "Confirmer"
    $('#productOffers').on('click', 'button:contains("Confirmer")', function () {
            var userId = $(this).data('userId');
            var productId = $(this).data('productId');
			var offreId = $(this).data('offreId');
			var isconfirmed = $(this).data('isconfirmed');
			var confirmButton = $(this); // Sélectionnez le bouton "Confirmer" actuel


$.ajax({
    type: 'POST',
	url: '/confirmEchange/' + offreId,
     data: {
            _token: '{{ csrf_token() }}',
        },
    success: function (response) {
        if (response.success) {
            alert('Échange confirmé avec succès !');
        } else {
            // Gérez les erreurs
        }
    },
    ...
    $(document).ready(function () {
        $('.js-edit-product').click(function (e) {
            e.preventDefault(); // Empêcher le lien de suivre son URL par défaut

            var productId = $(this).data('product-id');

            // Utilisez productId pour rediriger vers la page d'édition
            window.location.href = '{{ route('productseditf', ':id') }}'.replace(':id', productId);
        });
    });
